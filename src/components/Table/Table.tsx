...
export function Table<T>({ data, columns, isLoading, actions, className, sort, order, onSort }: TableProps<T>) {
      ...
      <UITable className="min-w-full">
        <TableHeader>
          <TableRow className="bg-muted/40 sticky top-0 z-10 border-b border-border">
            {columns.map((col, idx) => (
              ...
              >
                <span className="flex items-center gap-1">
                  {col.header}
                  {col.accessor && sort === col.accessor && (
            ...
            {actions && <TableHead className="px-4 py-2 font-semibold text-xs text-foreground uppercase tracking-wider bg-muted/40">{t('table.table_column_actions')}</TableHead>}
          </TableRow>
        </TableHeader>
        <TableBody>
          {isLoading ? (
            Array.from({ length: 5 }).map((_, i) => (
              <TableRow key={i} className="border-b border-border">
                ...
                {actions && <TableCell className="px-4 py-2"><Skeleton className="h-4 w-8 mx-auto" /></TableCell>}
              </TableRow>
            ))
          ) : data.length === 0 ? (
            <TableRow>
              <TableCell colSpan={columns.length + (actions ? 1 : 0)} className="text-center px-4 py-2 text-muted-foreground">{t('table.table_no_data')}</TableCell>
            </TableRow>
          ) : (
            data.map((row, i) => (
