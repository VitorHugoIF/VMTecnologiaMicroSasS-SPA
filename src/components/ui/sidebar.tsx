...
export function SidebarProvider({ children, ...props }: React.PropsWithChildren<any>) {
  ...
  return (
    <SidebarContext.Provider value={{ open, setOpen, openMobile, setOpenMobile, isMobile, toggleSidebar }} {...props}>
      {children}
...
export function SidebarHeader({ children, className }: { children?: React.ReactNode; className?: string }) {
  return <div className={cn("sticky top-0 z-10 flex items-center border-b bg-sidebar px-4 py-2", className)}>{children}</div>
}

export function SidebarFooter({ children, className }: { children?: React.ReactNode; className?: string }) {
  return <div className={cn("sticky bottom-0 z-10 flex items-center border-t bg-sidebar px-4 py-2", className)}>{children}</div>
}
...
export function SidebarGroupLabel({ children, className }: { children?: React.ReactNode; className?: string }) {
  return <div className={cn("px-2 py-1 text-xs font-semibold uppercase text-muted-foreground", className)}>{children}</div>
}
...
export function SidebarMenuBadge({ children, className }: { children?: React.ReactNode; className?: string }) {
  return <span className={cn("ml-2 rounded bg-primary px-2 py-0.5 text-xs font-semibold text-primary-foreground", className)}>{children}</span>
}
